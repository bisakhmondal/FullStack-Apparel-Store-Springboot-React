...
const airports = ["Kolkata (CCU)", "Delhi (DEL)", "Mumbai (BOM)", "Bangalore (BLR)", "Chennai (CAA)", "Hyderabad (HYD)", "Guwahati (GAU)"]
...
fs.writeFileSync("./airports.json", JSON.stringify(departures), 'utf8', (err)=>{
    if(err) throw new Error("Throw away ur pc")
})
console.log("JSON has been successfully saved to disk...")
